...
#
# Usage:
#   ./scripts/start.sh              Start core services (n8n, Qdrant, Postgres, Frontend)
#   ./scripts/start.sh --all        Start everything including Ollama + Grafana
#   ./scripts/start.sh --ollama     Start core + Ollama (local LLM)
#   ./scripts/start.sh --monitoring Start core + Grafana
#   ./scripts/start.sh --stop       Stop all services
#   ./scripts/start.sh --status     Show status of all services
#   ./scripts/start.sh --reset      Stop + delete all data (DESTRUCTIVE)
# ═══════════════════════════════════════════════════════════════════════════════
...
if [ ! -f "member5-frontend/Dockerfile" ]; then
  echo -e "  ${YELLOW}⚠ member5-frontend/Dockerfile not found. Frontend won't build.${NC}"
else
